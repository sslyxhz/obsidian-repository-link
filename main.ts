...
class InputRepositoryLinkModal extends Modal {
	result: string;
	...
	onOpen() {
		...
		contentEl.createEl("h2", { text: "Input Repository Link" });
		...
		new Setting(contentEl)
			...
			.then((setting) => {
				...
				if (inputEl) {
					inputEl.addClass("input-repository-link");
				}
			});

		const btnSubmit = new Setting(contentEl).addButton((btn) =>
			btn
